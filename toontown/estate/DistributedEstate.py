...
class DistributedEstate(DistributedObject.DistributedObject):
    ...
    def load(self):
        ...
        self.rightDoor.reparentTo(render)
        self.leftDoor.reparentTo(render)
        self.rightDoor.setPos(-106,-108.5626,0)
        self.leftDoor.setPos(-79.2,-108.5626,0)
        self.rightDoor.setScale(0.95)
        self.leftDoor.setScale(0.95)
        sphere = CollisionSphere(-93, -108.5626, 0, 20)
        sphere.setTangible(0)
        self.doorCollider = render.attachNewNode(CollisionNode("EstateBossDoor"))
        self.doorCollider.node().addSolid(sphere)
        self.doorCollider.node().setIntoCollideMask(ToontownGlobals.WallBitmask)
        self.accept("enterEstateBossDoor", self.checkBossDoor)
        self.accept("exitEstateBossDoor", self.checkBossDoorOff)

    def checkBossDoor(self, args):
        if not self.bossDoorOpen:
            self.bossDoorOpen = True
            rd = self.rightDoor.getPos()
            ld = self.leftDoor.getPos()
            seq = Sequence(LerpPosInterval(self.rightDoor, 2.0, Point3(-111.5,rd.y,rd.z)))
            seq2 = Sequence(LerpPosInterval(self.leftDoor, 2.0, Point3(-73.7,ld.y,ld.z)))
            seq.start()
            seq2.start()

    def checkBossDoorOff(self, args):
        if self.bossDoorOpen:
            self.bossDoorOpen = False
            rd = self.rightDoor.getPos()
            ld = self.leftDoor.getPos()
            seq = Sequence(LerpPosInterval(self.rightDoor, 2.0, Point3(-106,rd.y,rd.z)))
            seq2 = Sequence(LerpPosInterval(self.leftDoor, 2.0, Point3(-79.2,ld.y,ld.z)))
            seq.start()
            seq2.start()
    ...
    def unload(self):
        ...
        if self.elevatorModel:
            ...
            self.leftDoor.removeNode()
            self.rightDoor.removeNode()
            self.elevatorModel = None
            self.rightDoor = None
            self.leftDoor = None
